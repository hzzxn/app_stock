...
class UserService:
    ...
    def create_user(
        ...
        NOTA: No se permite crear usuarios con rol "China Import".
        Este rol es exclusivo del superusuario creado en inicialización.
        """
        ...
        # ═══════════════════════════════════════════════════════════════════
        # PROTECCIÓN: No permitir crear usuarios con rol China Import
        # Este rol es exclusivo del superusuario creado en inicialización
        # ═══════════════════════════════════════════════════════════════════
        if self.is_protected_role(role):
            return {
                'ok': False, 
                'error': f'El rol "{self.PROTECTED_ROLE}" no puede ser asignado. Solo existe un superusuario.'
            }
    ...
    def change_password(
        ...
        Args:
            ...
            admin_user: Admin que hace el cambio (para auditoría)
            clear_force_change: Si True, quita el flag force_password_change
        ...
        # Quitar el flag force_password_change si corresponde
        if success and clear_force_change:
            self.user_repo.clear_force_password_change(username)
    ...
    def needs_password_change(self, username: str) -> bool:
        """
        Verifica si un usuario necesita cambiar su contraseña.
        
        Args:
            username: Nombre de usuario
            
        Returns:
            True si force_password_change está activo
        """
        return self.user_repo.needs_password_change(username)
