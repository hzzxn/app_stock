# ==============================================================================
# CHINA IMPORT STOCK SYSTEM - Aplicación Principal
# ==============================================================================
# Aplicación Flask para gestión de inventario y ventas.
# Preparada para MySQL con arquitectura en capas.
# ==============================================================================
...
# ═══════════════════════════════════════════════════════════════════════════
from app_stock.app_container import get_container
...
def save_users(users):
    ...
    for u, v in users.items():
        serial[u] = {
            ...
            "role": normalize_role(v.get("role", "")),
            "force_password_change": v.get("force_password_change", False)
        }
...
def generate_secure_password(length=16):
    ...
    # Asegurar que tenga al menos: 1 mayúscula, 1 minúscula, 1 dígito, 1 especial
...
def load_users():
    ...
    FLUJO PRODUCCIÓN (users.json no existe):
    1. Genera contraseña temporal segura (random)
    2. Imprime la contraseña en logs (ÚNICA VEZ)
    3. Guarda hash en users.json con force_password_change=True
    ...
    if os.path.exists(USERS_FILE):
        try:
            with open(USERS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            # Cargar usuarios preservando force_password_change si existe
            users = {}
    ...
    if PRODUCTION_MODE:
        # En producción, crear archivo con China Import y contraseña temporal segura
        ...
        users = {
            "china": {
                ...
                "role": "China Import",
                "force_password_change": True  # OBLIGATORIO cambiar en primer login
            }
        }
        
        # ═══════════════════════════════════════════════════════════════════════════
        # IMPRIMIR CONTRASEÑA TEMPORAL - ÚNICA VEZ, NUNCA MÁS
        # ═══════════════════════════════════════════════════════════════════════════
        print("")
        print("═" * 70)
        print("  [PRODUCCIÓN] CUENTA CHINA IMPORT CREADA")
        print("═" * 70)
        print(f"  Usuario:              china")
        print(f"  Contraseña temporal:  {temp_password}")
        print("═" * 70)
        print("  ⚠️  IMPORTANTE: Esta contraseña SOLO se muestra UNA VEZ")
        print("  ⚠️  Debes cambiarla obligatoriamente en el primer login")
        print("  ⚠️  Guárdala en un lugar seguro AHORA")
        print("═" * 70)
        print("")
    else:
        # En desarrollo, crear usuarios de prueba
        users = {
            "admin": {"password": generate_password_hash("1234"), "role": "admin", "force_password_change": False},
            "operador": {"password": generate_password_hash("1234"), "role": "operador", "force_password_change": False},
            "china": {"password": generate_password_hash("changeme"), "role": "China Import", "force_password_change": False}
        }
...
def create_sale_from_cart(cart_items, user, client_data=None, payments=None, delivery=None):
    ...
    Args:
        cart_items: Lista de items del carrito:
            [{producto_id, cantidad, precio_unitario, variacion_id, uv, uv_label?, variant_attributes?}]
...
def login_required(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # BLOQUEO: Si tiene cambio de contraseña pendiente, no acceder
        # ═══════════════════════════════════════════════════════════════════
    ...
    try:
        if request.method == "POST":
            ...
            if user_rec and check_password_hash(user_rec["password"], password):
                ...
                # ═══════════════════════════════════════════════════════════════════
                # Si el usuario tiene force_password_change, redirigir a cambio de contraseña
                # No permitir acceso al sistema hasta que la cambie
                if user_rec.get("force_password_change", False):
                    session["force_password_change"] = True
                    flash("Debes cambiar tu contraseña antes de continuar.", "warning")
                    log_audit("login", user, details="Login con cambio de contraseña pendiente")
                    return redirect(url_for("force_change_password"))
...
# CAMBIO DE CONTRASEÑA OBLIGATORIO
# ═══════════════════════════════════════════════════════════════════════════════
# Esta ruta se usa cuando un usuario tiene force_password_change=True
# No puede acceder a ninguna otra parte del sistema hasta cambiar la contraseña
...
def force_change_password():
    ...
    2. El usuario tiene force_password_change=True
    
    Una vez cambiada:
    - Quita force_password_change
    - Permite acceso normal al sistema
    """
    ...
    if request.method == "POST":
        ...
        if not (has_upper and has_lower and has_digit):
            flash("La contraseña debe incluir mayúsculas, minúsculas y números.", "warning")
            return render_template("change_password.html", forced=True)
        ...
        # Quitar flag de sesión
        session.pop("force_password_change", None)
        
        flash("Contraseña actualizada correctamente. Bienvenido al sistema.", "success")
        log_audit("password_change", username, details="Cambio de contraseña obligatorio completado")
...
def create_product():
    ...
    INVENTARIO[pid] = {
        "sku": sku,
        "nombre": nombre,
        "cantidad": 0,  # Siempre 0, el stock real está en variantes
...
from app_stock.services.stats_service import get_stats_service
